...
if (recipeForm) {
    recipeForm.addEventListener("submit",async function (event) {
        ...
        if (title.length < 2 || description.length < 5 || !time ||!servings) {
            alert("Please fill in all required fields.");
            return;
        }
        const ingredients = [];
        document.querySelectorAll(".ingredient").forEach(function (input) {
        ...
        const instructions = [];
            document.querySelectorAll(".step").forEach(function (input) {
                const value =input.value.trim();
                    if (value) {
            ...
            try {
                ...
                alert(error.message ||"Could not create recipe.");
                if (submitButton) {
                    submitButton.disabled = false;
                    submitButton.textContent ="Publish Recipe";
                }
            }
        }
    );
