...
async function loadRecipe() {
    if (!recipeId) {
        showRecipeError("Invalid recipe.");
        return;
    }
    try {
...
function displayRecipe(recipe) {
    ...
    if (recipeImage) {
        recipeImage.src = getImageUrl(recipe.image);
    ...
    if (recipeAuthor) {
        recipeAuthor.textContent = "By " + (recipe.author || "Unknown");
    }
    if (recipeTime) {
        recipeTime.textContent = recipe.time + " mins" || "-";
    }
    if (recipeServings) {
...
async function loadFavouriteStatus() {
    ...
    try {
        ...
        console.error("Load favourite status error:",error);
    }
}
...
async function toggleFavourite() {
    ...
    try {
        const getResponse = await fetch(`http://localhost:3000/api/favourites/${userId}`);
        const favourites = await getResponse.json();
        if (!getResponse.ok) {
            alert(favourites.message || "Could not load favourites.");
            return;
        }
        ...
        if (alreadyFavourite) {
            ...
            const data =await response.json();
            if (!response.ok) {
                alert(data.message || "Could not add favourite.");
                return;
            }
            updateHeart(true);
        }
