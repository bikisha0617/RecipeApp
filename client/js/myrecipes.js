...
function displayRecipes(recipeList) {
    ...
    if (!recipeList || recipeList.length === 0) {
        recipeGrid.innerHTML = `
            <p class="empty-message">You haven't created any recipes yet.</p>
        `;
        return;
    }
    recipeList.forEach(function (recipe) {
        ...
        recipeGrid.innerHTML += `
            <div class="recipe-box">
                <img src="${imageUrl}" alt="${recipe.title || "Recipe"}" class="recipe-image" onerror="this.onerror=null; this.src='images/placeholder.jpg';">
                <div class="recipe-info">
                    <h3>${recipe.title || ""}</h3>
                    <p>${recipe.time || "-"} mins</p>
                    <div class="recipe-actions">
                        <button type="button" class="view-btn" onclick="viewRecipe(${Number(recipe.id)})">View</button>
                        <button type="button" class="delete-btn" onclick="deleteRecipe(${Number(recipe.id)})">Delete</button>
                    </div>
                </div>
            </div>
        `;
...
async function loadRecipes() {
    ...
    try {
        ...
        userRecipes = Array.isArray(data) ? data : [];
...
async function deleteRecipe(id) {
    ...
    try {
        const response =await fetch(`http://localhost:3000/api/recipes/${Number(id)}`,
            {
                method: "DELETE"
            }
        );
