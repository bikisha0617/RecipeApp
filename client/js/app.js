...
function displayRecipes(recipeList) {
    ...
    recipeList.forEach(function (recipe) {
        ...
        const author =escapeHtml(recipe.author || "Recipe App");
        recipeContainer.innerHTML += `
            <div class="recipe-card">
                <a href="recipe.html?id=${Number(recipe.id)}">
                    <img src="${imageUrl}" class="recipe-image" alt="${title}" 
...
async function loadRecipes() {
    ...
    try {
        ...
        if (!Array.isArray(recipes)) {
            console.error("Invalid recipe response:",recipes);
            displayError("The server returned an invalid recipe list.");
            return;
        }
        allRecipes = recipes;
        ...
        displayError(error.message || "Could not connect to the server.");
    }
}
if (searchInput) {
    searchInput.addEventListener("input",function () {
        ...
        if (filteredRecipes.length === 0) {
            displayEmpty(`No recipes found for "${keyword}".`);
            return;
        }
