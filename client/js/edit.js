...
async function loadRecipe() {
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error(data.message ||"Could not load recipe.");
        }
        originalRecipe =data;
        ...
        loading.style.display ="none";
        showMessage(error.message || "Could not load recipe.","error");
    }
}
...
form.addEventListener("submit",async function (event) {
    ...
    const instructions =getInputValues(".instruction-input");
        /* Validation */
    if (title.length < 2) {
        showMessage("Recipe title must be at least 2 characters.","error");
        return;
    }
    if (title.length > 150) {
        showMessage("Recipe title cannot exceed 150 characters.","error");
        return;
    }
    if (description.length < 5) {
        showMessage("Recipe description must be at least 5 characters.","error");
        return;
    }
    if (!time) {
        showMessage("Cooking time is required.","error");
        return;
    }
    if (!servings) {
        showMessage("Servings are required.","error");
        return;
    }
    ...
    try {
        const response =await fetch("http://localhost:3000/api/recipes/" +recipeId,
            ...
            if (!response.ok) {
                if (response.status === 401 || response.status === 403) {
                    showMessage(data.message ||"You are not allowed to edit this recipe.","error");
                    return;
                }
                throw new Error(data.message || "Could not update recipe.");
            }
            showMessage(data.message || "Recipe updated successfully.", "success");
            setTimeout(function () {
                window.location.href ="myrecipes.html";
            },800
        );
        ...
        showMessage(error.message ||"Could not update recipe.","error");
    } finally {
