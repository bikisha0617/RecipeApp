document.addEventListener("DOMContentLoaded", function () {
    ...
    function getImageUrl(image) {
        ...
        if (cleanImage.startsWith("images/") || cleanImage.startsWith("./images/")) {
            return cleanImage;
        }
            ...
            favouriteGrid.innerHTML += `
                <div class="recipe-card">
                    <a href="recipe.html?id=${recipeId}">
                        <img src="${image}" class="recipe-image" alt="${title}"
                    ...
                    <div class="recipe-content">
                        ...
                        <div class="buttons">
                            <button type="button" class="view-btn" onclick="viewFavouriteRecipe(${recipeId})"> View </button>
                            <button type="button" class="remove-btn" onclick="removeFavouriteRecipe(${recipeId})"> Remove </button>
                        </div>
                    </div>
                </div>
            `;
        ...
        try {
            ...
            favouriteGrid.innerHTML = `
                <div class="empty-state">
                    ...
                    <button type="button" onclick="loadFavouriteRecipesAgain()">Try Again</button>
                </div>
            `;
        }
    }
    ...
    window.removeFavouriteRecipe = async function (id) {
        ...
        const recipe =favouriteRecipes.find(function (item) {
            return Number(item.id) === recipeId;
        });
        const recipeTitle =recipe && recipe.title ? recipe.title : "this recipe";
        const confirmed = confirm('Remove "' +recipeTitle +'" from your favourites?');
        if (!confirmed) {
            return;
        }
        const token = getToken();
        ...
        try {
            const response = await fetch("http://localhost:3000/api/favourites/" +recipeId,
                {
                    method: "DELETE",
                    headers: {
                        "Authorization": "Bearer " + token
            ...
            if (response.status === 401 || response.status === 403) {
                alert(data.message || "You are not authorized to remove this favourite.");
                return;
            }
            if (!response.ok) {
                throw new Error(data.message || "Could not remove favourite.");
            }
            ...
            alert(data.message || "Recipe removed from favourites.");
        } catch (error) {
            console.error("Remove favourite error:", error);
            alert(error.message || "Could not remove favourite.");
        }
    };
    ...
    window.loadFavouriteRecipesAgain =function () {
        loadFavourites();
    };
