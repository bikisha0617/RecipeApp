...
const {User,Admin,Recipe,Ingredient,Instruction,Favourite} = require("../models");
...
router.get("/dashboard",authenticateToken,requireAdmin,
    async function (req, res) {
        try {
            ...
            const favouriteCount =await Favourite.count();
...
router.get("/recipes",authenticateToken,requireAdmin,
    async function (req, res) {
        try {
...
router.get("/users/:id",authenticateToken,requireAdmin,
    async function (req, res) {
        try {
...
router.delete("/recipes/:id",authenticateToken,requireAdmin,
    async function (req, res) {
        ...
        try {
            ...
            await Ingredient.destroy({
                where: {recipeId: recipeId},transaction
            });
            ...
            if (transaction) {
                try {
                    ...
                    console.error("Admin recipe rollback error:",rollbackError);
                }
            }
    ...
    async function (req, res) {
        ...
        try {
            ...
            if (recipeIds.length > 0) {
                await Ingredient.destroy({
                    where: {recipeId: recipeIds},transaction
                });
                await Instruction.destroy({
                    where: {recipeId: recipeIds},transaction
                });
                ...
                await Recipe.destroy({
                    where: {id: recipeIds},transaction
                });
            }
            await Favourite.destroy({
            ...
            if (transaction) {
                try {
                    ...
                    console.error("Admin user rollback error:",rollbackError);
                }
            }
