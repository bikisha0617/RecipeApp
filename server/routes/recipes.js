...
function formatRecipe(recipe) {
    ...
    if (Array.isArray(result.instructions)) {
        result.instructions = result.instructions.map(
            function (item) {
...
router.get("/user/:userId",authenticateToken,requireUser,
...
router.post("/",authenticateToken,requireUser,upload.single("image"),
    async function (req, res) {
        ...
        await Instruction.bulkCreate(
            instructionArray.map(function (instruction) {
...
router.put("/:id",authenticateToken,requireUser,
    async function (req, res) {
        ...
        try {
            ...
            if (Number(recipe.userId) !==Number(req.user.id)) {
                return res.status(403).json({
            ...
            let ingredientArray = null;
            let instructionArray = null;
            if (req.body.ingredients !==undefined) {
                ingredientArray =cleanStringArray(parseArray(req.body.ingredients));
                if (ingredientArray.length === 0) {
                    return res.status(400).json({
                        message:"At least one ingredient is required."
                    });
            ...
            if (req.body.instructions !==undefined) {
                instructionArray =cleanStringArray(parseArray(req.body.instructions));
                if (instructionArray.length === 0) {
                    return res.status(400).json({
                        message:"At least one instruction is required."
                    });
            ...
            if (ingredientArray) {
                ...
                await Ingredient.bulkCreate(ingredientArray.map(
                    function (ingredient) {
                        return {recipeId:recipe.id,ingredient:ingredient};
                    }
                ),
            ...
            if (instructionArray) {
                ...
                await Instruction.bulkCreate(instructionArray.map(
            ...
            return res.json({message:"Recipe updated successfully.",recipe:formatRecipe(updatedRecipe)});
        } catch (error) {
            if (transaction) {
                try {
                    await transaction.rollback();
                } catch (rollbackError) {
                    console.error("Update rollback error:",rollbackError);
                }
            }
